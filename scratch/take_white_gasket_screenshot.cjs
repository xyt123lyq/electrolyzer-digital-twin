...
(async () => {
  ...
  console.log('Isolating thinWhite1 and positioning camera...');
  await page.evaluate(async () => {
    const scene = window.__scene__;
    scene.stopAutoDemo();
    scene.resetAll();
    
    // Hide all layers except thinWhite1
    scene.cell.children.forEach(child => {
      child.visible = (child.name === 'thinWhite1');
    });
    
    // Position camera close up, looking straight down
  ...
  const savePath = path.resolve(__dirname, '../white_gasket_close.png');
  console.log(`Saving thin-white close-up screenshot to: ${savePath}`);
